...
char *read_file(char *filename)
{
    ...
    {
        fprintf(stderr, "failure stat file\n");
        return NULL;
    }
    FILE *fp = fopen(filename, "rb");
    char *f = (char *)malloc(sizeof(char) * st.st_size);
    fread(f, st.st_size, 1, fp);
    /*
    int fd;
    fd = open(filename, O_RDONLY);
    if(fd < 0){
        fprintf(stderr, "failure open the file \n");
        return NULL;
    }
    char *f;
    f = (char*)mmap(NULL, st.st_size, PROT_READ, MAP_PRIVATE, fd, 0);
    if(f == (void*)-1){
        fprintf(stderr, "failure mapping file to process \n");
        return NULL;
    }
	*/
    split_string(f);
...
int main(int argc, char *argv[])
{
    clock_t t;
    t = clock();
    read_file("resources/list.txt");
    t = clock() - t;
    double time_taken = ((double)t) / CLOCKS_PER_SEC; // in seconds
    printf("read file took %f seconds to execute \n", time_taken);
    t = clock();
    int_radix_sort(arr, MAX);
    t = clock() - t;
    time_taken = ((double)t) / CLOCKS_PER_SEC; // in seconds
    printf("sort took %f seconds to execute \n", time_taken);
    t = clock();
    FILE *out = fopen("out/out.txt", "wb");
    for (int i = 0; i < MAX; ++i)
    {
        fprintf(out, "%d\n", arr[i]);
    }
    ...
    t = clock() - t;
    time_taken = ((double)t) / CLOCKS_PER_SEC; // in seconds
    printf("write file took %f seconds to execute \n", time_taken);
    return 0;
}
